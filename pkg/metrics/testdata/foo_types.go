...
type FooStatus struct {
	// +k8s:controller-gen:metrics:stateset:name="status_condition",help="The condition of a foo.",labelName="status",JSONPath=".status",list={"True","False","Unknown"},labelsFromPath={"type":".type"}
	// +k8s:controller-gen:metrics:gauge:name="status_condition_last_transition_time",help="The condition last transition time of a foo.",valueFrom=.lastTransitionTime,labelsFromPath={"type":".type","status":".status"}
	Conditions []Condition `json:"conditions,omitempty"`
}

// Foo is a test object.
// +k8s:controller-gen:metrics:gvk:namePrefix="foo"
// +k8s:controller-gen:metrics:labelFromPath:name="name",JSONPath=".metadata.name"
// +k8s:controller-gen:metrics:gauge:name="created",JSONPath=".metadata.creationTimestamp",help="Unix creation timestamp."
// +k8s:controller-gen:metrics:info:name="owner",JSONPath=".metadata.ownerReferences",help="Owner references.",labelsFromPath={owner_is_controller:".controller",owner_kind:".kind",owner_name:".name",owner_uid:".uid"}
// +k8s:controller-gen:metrics:labelFromPath:name="cluster_name",JSONPath=.metadata.labels.cluster\.x-k8s\.io/cluster-name
type Foo struct {
