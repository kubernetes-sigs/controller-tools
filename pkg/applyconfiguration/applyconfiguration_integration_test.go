...
var _ = Describe("ApplyConfiguration OmitToolVersion", func() {
	var originalCWD string
	var tmpDir string
	...
	BeforeEach(func() {
		By("Setting up a temporary directory", func() {
			var err error
			tmpDir, err = os.MkdirTemp("", "applyconfiguration-version-test")
			Expect(err).NotTo(HaveOccurred())
	...
	It("should leave the generator name unchanged when OmitToolVersion is unset", func() {
		output := make(outputToMap)
		...
		By("Checking the generated file header preserves the original generator name")
		generatedFile := filepath.Join("api/v1", applyConfigurationDir, "api/v1/cronjob.go")
		content, err := os.ReadFile(generatedFile)
		Expect(err).NotTo(HaveOccurred())

		Expect(string(content)).To(HavePrefix("// Code generated by "))
		Expect(string(content)).NotTo(HavePrefix("// Code generated by controller-gen. DO NOT EDIT."))
	})

	It("should normalize generator name to controller-gen when OmitToolVersion is true", func() {
		output := make(outputToMap)
		...
		rt, err := genall.FromOptions(optionsRegistry, []string{
			"applyconfiguration:omitToolVersion=true,externalApplyConfigurations=sigs.k8s.io/controller-tools/pkg/applyconfiguration/testdata/cronjob/external.ExternalData@sigs.k8s.io/controller-tools/pkg/applyconfiguration/testdata/cronjob/externalac",
			"paths=./api/v1",
		})
		Expect(err).NotTo(HaveOccurred())
		...
		By("Checking the generated file header")
		generatedFile := filepath.Join("api/v1", applyConfigurationDir, "api/v1/cronjob.go")
		content, err := os.ReadFile(generatedFile)
		Expect(err).NotTo(HaveOccurred())
