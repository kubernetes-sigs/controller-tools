...
var _ = Describe("ApplyConfiguration SkipToolVersion", func() {
	var originalCWD string
	var tmpDir string
	...
	It("should normalize generator name to controller-gen when SkipToolVersion is true", func() {
		output := make(outputToMap)
		...
		rt, err := genall.FromOptions(optionsRegistry, []string{
			"applyconfiguration:skipToolVersion=true,externalApplyConfigurations=sigs.k8s.io/controller-tools/pkg/applyconfiguration/testdata/cronjob/external.ExternalData@sigs.k8s.io/controller-tools/pkg/applyconfiguration/testdata/cronjob/externalac",
			"paths=./api/v1",
		})
		Expect(err).NotTo(HaveOccurred())
