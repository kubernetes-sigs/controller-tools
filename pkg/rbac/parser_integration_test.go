...
var _ = Describe("ClusterRole generated by the RBAC Generator", func() {
	...
	for i := 0; i < stableTestCount; i++ {
		It("should match the expected result for loading directory", func() {
			By("switching into testdata to appease go modules")
			cwd, err := os.Getwd()
			Expect(err).NotTo(HaveOccurred())
			Expect(os.Chdir("./testdata/directory")).To(Succeed()) // go modules are directory-sensitive
	...
	for i := 0; i < stableTestCount; i++ {
		It("should match the expected result for loading single file", func() {
			By("switching into testdata to appease go modules")
			cwd, err := os.Getwd()
			Expect(err).NotTo(HaveOccurred())
			Expect(os.Chdir("./testdata/file")).To(Succeed()) // go modules are directory-sensitive
			defer func() { Expect(os.Chdir(cwd)).To(Succeed()) }()

			By("loading the roots")
			pkgs, err := loader.LoadRoots("controller.go")
			Expect(err).NotTo(HaveOccurred())
