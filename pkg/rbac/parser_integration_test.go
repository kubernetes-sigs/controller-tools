...
var _ = Describe("ClusterRole generated by the RBAC Generator", func() {
	...
	It("should write a manifest without creationTimestamp", func() {
		By("switching into testdata to appease go modules")
		cwd, err := os.Getwd()
		Expect(err).NotTo(HaveOccurred())
		Expect(os.Chdir("./testdata")).To(Succeed()) // go modules are directory-sensitive
		...
		By("running the generator into a temporary directory")
		...
		Expect(rbac.Generator{RoleName: "manager-role"}.Generate(ctx)).To(Succeed())

		By("comparing the written manifest with the expected one")
		actualFile, err := os.ReadFile(path.Join(outputDir, "role.yaml"))
		Expect(err).NotTo(HaveOccurred())
		expectedFile, err := os.ReadFile("role.yaml")
		Expect(err).NotTo(HaveOccurred())
		Expect(string(actualFile)).To(Equal(string(expectedFile)), "check pkg/rbac/testdata/README.md for more details.\n\nDiff:\n\n%s", cmp.Diff(string(actualFile), string(expectedFile)))
	})
})
