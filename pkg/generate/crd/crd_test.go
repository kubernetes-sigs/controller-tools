...
func TestGenerateCRD(t *testing.T) {
	...
	testDir := filepath.Join(filepath.Dir(filepath.Dir(filepath.Dir(currDir))), "test", "testData")
	o := &Options{
		OutFs:     outFs,
		OutputDir: "/tmp",
		RootPath:  testDir,
	...
	for _, f := range []string{"fun_v1alpha1_toy.yaml"} {
		...
		expectedContent, err := ioutil.ReadFile(filepath.Join(testDir, "config", "crds", f))
		if err != nil {
			t.Fatalf("reading file failed %v", err)
		}
