...
var _ = Describe("CRD Generation From Parsing to CustomResourceDefinition", func() {
	...
	It("should skip unexported field", func() {
		By("switching into testdata to appease go modules")
		cwd, err := os.Getwd()
		Expect(err).NotTo(HaveOccurred())
		Expect(os.Chdir("./testdata/unexported")).To(Succeed())
		defer func() { Expect(os.Chdir(cwd)).To(Succeed()) }()
		...
		By("requesting that the CRD be generated")
		groupKind := schema.GroupKind{Kind: "TestQuota", Group: "unexported.example.com"}
		parser.NeedCRDFor(groupKind, nil)
		...
		By("loading the desired YAML")
		expectedFile, err := ioutil.ReadFile("unexported.example.com_testquotas.yaml")
		Expect(err).NotTo(HaveOccurred())
