...
func (XValidation) Help() *markers.DefinitionHelp {
	return &markers.DefinitionHelp{
		...
		FieldHelp: map[string]markers.DetailedHelp{
			"Rule": {
				Summary: "represents the expression which will be evaluated by CEL.",
				Details: "The Rule is scoped to the location of the XValidation in the schema.\nThe `self` variable in the CEL expression is bound to the scoped value.\nExample: Rule=\"self.minReplicas <= self.replicas && self.replicas <= self.maxReplicas\"",
			},
			"Message": {
				Summary: "represents the message displayed when validation fails.",
				Details: "If message is unset, a generic default validation message will be provided.\nMessage may be up to 256 characters in length.\nExample: Message=\"replicas must be between minReplicas and maxReplicas\"",
			},
			"MessageExpression": {
				Summary: "declares a CEL expression that evaluates to the validation failure message.",
				Details: "MessageExpression and Message are mutually exclusive. MessageExpression must produce a string.\nIf both are set, MessageExpression will be used. If neither is set, the default message is used.\nThe expression has access to the same CEL variables as the Rule.\nExample: MessageExpression=\"'replicas must be between ' + string(self.minReplicas) + ' and ' + string(self.maxReplicas)\"",
			},
			"Reason": {
				Summary: "provides a machine-readable validation failure reason that is returned to the caller",
				Details: "when a request fails this validation rule. HTTP clients can distinguish between different\nvalidation failures based on the reason. Currently supported reasons are: \"FieldValueInvalid\",\n\"FieldValueForbidden\", \"FieldValueRequired\", \"FieldValueDuplicate\".\nIf not set, a generic \"Invalid\" reason will be used.\nExample: Reason=\"FieldValueInvalid\"",
			},
			"FieldPath": {
				Summary: "represents the path from the validation failure back to the object's field.",
				Details: "It is used to indicate which field in the validated object caused the failure.\nExample: FieldPath=\".spec.replicas\"",
			},
			"OptionalOldSelf": {
				Summary: "indicates that the oldSelf value is made optional. This is useful for",
				Details: "validation rules that need to work both on create and update operations without having\nto check if oldSelf is set. When OptionalOldSelf is set to true, the rule will be\nevaluated only when oldSelf is non-null.\nExample: OptionalOldSelf=true",
			},
		},
	}
}
